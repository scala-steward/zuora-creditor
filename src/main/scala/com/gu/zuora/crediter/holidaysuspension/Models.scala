...
object Models {
  ...
  case object NegativeHolidayCreditInvoicesExportCommand extends ExportCommand {
    ...
    override val getJSON: SerialisedJson = {
      ...
      JsObject(
        "Format" -> JsString("csv"),
        "Name" -> JsString(s"Negative Holiday Credit Invoices Export - ${now.format(ISO_DATE_TIME)}"),
